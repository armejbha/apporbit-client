import { FaUser, FaEnvelope, FaCalendarCheck, FaClock, FaUserShield } from 'react-icons/fa';
...
<div className='grid grid-cols-1 md:grid-cols-2 gap-4 w-full mt-6 text-gray-800'>
  ...
  <div className='flex items-center gap-3 bg-gray-100 p-4 rounded-lg'>
    ...
    <div>
      ...
      <p className='font-semibold'>{formatDate(user.metadata?.lastSignInTime)}</p>
    </div>
  </div>
  ...
  <div className='flex items-center gap-3 bg-gray-100 p-4 rounded-lg'>
    ...
    <div>
      ...
      <p className='font-semibold'>{formatDate(user.metadata?.creationTime)}</p>
    </div>
  </div>
</div>
